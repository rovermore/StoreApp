...
class CartRepositoryTest {
    ...
    fun `when saveProducts success productLocalDatasource saveProducts is called`() {
        Mockito.`when`(productLocalDatasource.saveProducts(productSelection)).thenReturn(Success(true))
        cartRepository.saveProducts(productSelection)
        Mockito.verify(productLocalDatasource, Mockito.times(1)).saveProducts(productSelection)
    }
    @Test
    ...
    fun `when saveProducts failure productLocalDatasource saveProducts returns error`() {
        Mockito.`when`(productLocalDatasource.saveProducts(productSelection)).thenReturn(error)
        val result = cartRepository.saveProducts(productSelection)
    ...
    fun `when getProducts success productLocalDatasource retrieveProducts is called`() {
        Mockito.`when`(productLocalDatasource.retrieveProducts()).thenReturn(successProductsDTO)
